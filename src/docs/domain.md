...
- 사용자는 카테고리 또는 명령 버튼 우클릭 메뉴에서 About 창을 열어 Cargo 패키지 버전 기반의 제품 버전 라벨과 `about.txt`의 프로그램 이름, 저작권 표시, GPL-3.0-or-later 배포 문구, 라이선스 전문 경로, 대응 소스코드 제공 안내를 확인할 수 있어야 한다.
- 설치된 앱은 실행 파일 옆 `about.txt`를 우선 읽고, 파일이 없으면 빌드에 포함된 동일한 기본 텍스트를 표시해야 한다.
...
- `ApplicationIdentity`: 사용자에게 표시하는 앱 이름, Cargo 패키지 버전, 저작권 표시, GPL-3.0-or-later 라이선스 정보, 작성자 GitHub 프로필 링크를 묶은 제품 정보다. About 창은 Cargo 패키지 버전으로 만든 상단 제품 버전 라벨과 `about.txt` 또는 빌드 내장 기본 텍스트로 이 정보를 표시한다.
- `ThirdPartyLicenseNotice`: 외부 Rust crate, native 런타임 의존성, 아이콘/문서 리소스의 이름, 버전, 라이선스, 저작권자 또는 authors, 원본 URL, 라이선스 파일 위치, 배포 포함 여부를 기록한 배포 고지 문서다.
...
- About 표시: 플랫폼 UI가 Cargo 패키지 버전 기반의 `j3Term <version>` 라벨과 `about.txt` 내용을 표시하는 모달 About 창을 연다. 설치된 앱에서는 실행 파일 옆 `about.txt`를 우선 읽고, 없으면 빌드에 포함된 기본 텍스트를 표시한다. 사용자가 작성자 GitHub 프로필 링크를 클릭하면 기본 브라우저로 해당 URL을 연다. 이 흐름은 앱 상태나 설정 파일을 변경하지 않는다.
...
- `WM_CONTEXTMENU`: 카테고리 콤보박스와 명령 버튼 child HWND에서 온 우클릭 요청을 팝업 메뉴로 처리한다. 메뉴 ID는 Win32 어댑터 내부에만 두고 앱 계층에는 도메인 명령으로 전달한다. 폰트 설정 항목은 `ChooseFontW` 공용 대화상자를 열어 고정폭 폰트와 크기를 선택하게 하고, About 항목은 `about.txt` 내용과 클릭 가능한 GitHub 링크를 포함한 정보 창을 연다.
